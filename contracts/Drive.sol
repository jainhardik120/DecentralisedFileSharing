...
contract Drive {
    ...
    function transfer(address newOwner, string memory file) public {
        require(
            msg.sender == fileOwners[file],
            "You are not the owner of file"
        );
        bool found = false;
        for (uint i = 0; i < access[file].length; i++) {
            if (access[file][i] == newOwner) {
                found = true;
                break;
            }
        }
        ...
        ownedFiles[msg.sender][index] = ownedFiles[msg.sender][
            ownedFiles[msg.sender].length - 1
        ];
    ...
    function revokeAccess(address user, string memory file) public {
        ...
        require(found == true, "File not shared with user");
        sharedFiles[user][index] = sharedFiles[user][
            sharedFiles[user].length - 1
        ];
